...
class ViTDataset(Dataset):#PyTorch要求所有数据集类必须继承torch.utils.data.Dataset
    ...
    def __init__(self,root,split,transform=None,target_transform=None, img_size=224):
        ...
        # 构建数据集根目录
        self.data_dir=os.path.join(root,split)  # 构建训练集或测试集目录
        
        # 获取所有类别
        self.classes=sorted(os.listdir(self.data_dir))
        self.class_to_idx={cls_name:i for i,cls_name in enumerate(self.classes)}# 类别名到数字索引的映射
